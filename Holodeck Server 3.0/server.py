...
class XmlRelay(DatagramProtocol):
        ...
        def datagramReceived(self, data, (in_host, in_port)):
                ...
                if not (update_version > self.updates.get(update_id, (0, ''))):
                        return
                self.updates[update_version] = (update_version, data)
        ...
        def sendXml(self):
                ...
                if self.modication_version != self.rendered_version:
                        updates_xml = (u[1] for u in self.updates.values())
...
relay = XmlRelay()
reactor.listenUDP(1610, relay, interface='192.168.1.11', maxPacketSize=65507)
reactor.run()
